...
function ProductModal({ product, businessId, onClose, onSaved }: { product?: Product; businessId: string; onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      console.error("createProduct error:", caught);
      if (msg.toLowerCase().includes("duplicate") || msg.toLowerCase().includes("unique")) {
        setError("That SKU or IMEI already exists in this business.");
      } else if (msg.toLowerCase().includes("not allowed") || msg.toLowerCase().includes("permission") || msg.toLowerCase().includes("rls")) {
        setError("Permission denied. Make sure you are logged in as owner or manager.");
      } else if (msg.toLowerCase().includes("not configured") || msg.toLowerCase().includes("supabase")) {
        setError("Database not connected. Check your Supabase environment variables.");
      } else {
